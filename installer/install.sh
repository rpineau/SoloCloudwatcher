...
cp "./weatherstationlist SoloCloudwatcher.txt" "$TheSkyX_Path/Resources/Common/Miscellaneous Files/"
cp "./libSoloCloudwatcher.so" "$TheSkyX_Path/Resources/Common/$PLUGINS_DIR/WeatherStationPlugIns/"
cp "./SoloCloudwatcher.ui" "$TheSkyX_Path/Resources/Common/$PLUGINS_DIR/WeatherStationPlugIns/"

app_owner=`/usr/bin/stat -c "%u" "$TheSkyX_Path" | xargs id -n -u`
if [ ! -z "$app_owner" ]; then
	chown $app_owner "$TheSkyX_Path/Resources/Common/Miscellaneous Files/weatherstationlist SoloCloudwatcher.txt"
	chown $app_owner "$TheSkyX_Path/Resources/Common/$PLUGINS_DIR/WeatherStationPlugIns/libSoloCloudwatcher.so"
	chown $app_owner "$TheSkyX_Path/Resources/Common/$PLUGINS_DIR/WeatherStationPlugIns/SoloCloudwatcher.ui"
fi
chmod  755 "$TheSkyX_Path/Resources/Common/$PLUGINS_DIR/WeatherStationPlugIns/libSoloCloudwatcher.so"
